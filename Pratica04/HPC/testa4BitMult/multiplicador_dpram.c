...
static uint32_t calculate_mult4(peripheral *dpram, uint32_t A, uint32_t B)
{
    ...
    printf("peripheral_write32(*dpram, CONTROL_REGISTER_ADDR, 0x00)\n");
    peripheral_write32(*dpram, CONTROL_REGISTER_ADDR, 0x00);

    // Escreve os registradores A e B
    printf("peripheral_write32(*dpram, DATA_IN_REGISTER_ADDR, (B<<4) | A)\n");
    peripheral_write32(*dpram, DATA_IN_REGISTER_ADDR, (B<<4) | A);

    // Inicia o cálculo do produto de A e B
    peripheral_write32(*dpram, CONTROL_REGISTER_ADDR, 0x01);
    printf("peripheral_write32(*dpram, CONTROL_REGISTER_ADDR, 0x01)\n");
    
    // Aguarda o final do cálculo
    printf("Aguardo o final do calculo...\n");
    printf("peripheral_read32(*dpram, STATUS_REGISTER_ADDR)\n");
    while (1) {
        uint8_t completed = peripheral_read32(*dpram, STATUS_REGISTER_ADDR);
//	usleep(100*1000);
        if (completed) break;
    }

    // Lê o resultado
    uint32_t Y = peripheral_read32(*dpram, DATA_OUT_REGISTER_ADDR);
    printf("peripheral_read32(*dpram, DATA_OUT_REGISTER_ADDR) == %u\n", Y);
...
int main(int argc, char **argv)
{
    ...
    printf("Configurando acesso a� Dual Port RAM...\n");
