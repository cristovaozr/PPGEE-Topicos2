...
static uint32_t calculate_mult4(peripheral *dpram, uint32_t A, uint32_t B)
{
    ...
    printf("peripheral_write8(*dpram, CONTROL_REGISTER_ADDR, 0x00)\n");
    peripheral_write8(*dpram, CONTROL_REGISTER_ADDR, 0x00);

    // Escreve os registradores A e B
    printf("peripheral_write8(*dpram, DATA_IN_REGISTER_ADDR, (B<<4) | A)\n");
    ...
    printf("peripheral_write8(*dpram, CONTROL_REGISTER_ADDR, 0x01)\n");
    
    // Aguarda o final do cálculo
    printf("Aguardo o final do calculo...\n");
    printf("peripheral_read8(*dpram, STATUS_REGISTER_ADDR)\n");
    while (1) {
...
//	usleep(100*1000);
    ...
    printf("peripheral_read8(*dpram, DATA_OUT_REGISTER_ADDR) == %u\n", Y);
